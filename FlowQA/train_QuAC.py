...
parser.add_argument('--start_dialog_ctx', type=int, default=0)
parser.add_argument('--explicit_dialog_ctx', type=int, default=2)
...
def main():
    ...
    if args.resume:
        batches = BatchGen_QuAC(dev, batch_size=args.batch_size, evaluation=True, gpu=args.cuda, start_dialog_ctx=args.start_dialog_ctx,dialog_ctx=args.explicit_dialog_ctx, use_dialog_act=args.use_dialog_act, prob_shuffle=args.prob_shuffle)
    ...
    for epoch in range(epoch_0, epoch_0 + args.epoches):
        ...
        # train
        batches = BatchGen_QuAC(train, batch_size=args.batch_size, gpu=args.cuda, start_dialog_ctx=args.start_dialog_ctx,dialog_ctx=args.explicit_dialog_ctx, use_dialog_act=args.use_dialog_act, precompute_elmo=args.elmo_batch_size // args.batch_size, prob_shuffle=args.prob_shuffle)
        start = datetime.now()
        ...
        if epoch % args.eval_per_epoch == 0:
            batches = BatchGen_QuAC(dev, batch_size=args.batch_size, evaluation=True, gpu=args.cuda, start_dialog_ctx=args.start_dialog_ctx,dialog_ctx=args.explicit_dialog_ctx, use_dialog_act=args.use_dialog_act, precompute_elmo=args.elmo_batch_size // args.batch_size)
            predictions, no_ans_scores = [], []
            for batch in batches:
                phrases, noans = model.predict(batch)
                predictions.extend(phrases)
