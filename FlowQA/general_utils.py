...
#     rows, contexts,question_text = [], [],[]
#     i=0
#     for i in range(len(data)):
#     # if i<3:
#         i=i+1
#         if i<3:
#             partial_rows, context ,question_text= proc_func(i, data[i])
#             rows.extend(partial_rows)
#             contexts.append(context)
#     #         question_text.extend(question_text)
#     #         # print('question_text',question_text)
#     # print('0',question_text[0]) 
#     # print('1',question_text[1]) 
#     # print('0',rows[0])
#     # print('1',rows[1]) 
...
# #8.17
def flatten_json_823(file, proc_func):
    with open(file, encoding="utf8") as f:
        data = json.load(f)['data']
    rows, contexts ,qcontextss= [], [], []
    for i in range(len(data)):
        partial_rows, context ,qcontexts= proc_func(i, data[i])
        rows.extend(partial_rows)
        contexts.append(context)
        qcontextss.append(qcontexts)
    # print('0',rows[0])
    print('1',rows[1]) 
    # print('0',contexts[0])
    print('1',contexts[1]) 
    # print('0',qcontextss[0])
    # print('1',qcontextss[1])
    return rows, contexts,qcontextss
...
class BatchGen_CoQA:
    def __init__(self, data, batch_size, gpu, start_dialog_ctx=0,dialog_ctx=0, evaluation=False, context_maxlen=100000, precompute_elmo=0,
                 prob_shuffle=0.0):
        '''
        input:
            data - see train.py
        ...
        self.start_dialog_ctx = start_dialog_ctx
        self.dialog_ctx = dialog_ctx
        ...
        for batch_i in range((self.context_num + self.batch_size - 1) // self.batch_size):
            ...
            for i, q_seq in enumerate(question_batch):
                for j, id in enumerate(q_seq):
                    ...
                    #改：输入句子[0,1,2,3]
                    #for prv_ctx in range(0, self.dialog_ctx):(0,1,2) (0)(1)(2)

                    for prv_ctx in range(self.start_dialog_ctx, self.dialog_ctx):
                        if j > prv_ctx:
                            prv_id = id - prv_ctx - 1
...
class BatchGen_QuAC:
    def __init__(self, data, batch_size, gpu, start_dialog_ctx=0,dialog_ctx=0, use_dialog_act=False, evaluation=False, context_maxlen=100000, precompute_elmo=0,
                 prob_shuffle=0.0):
        '''
        input:
            data - see train.py
    ...
    def __len__(self):
        a=(self.context_num + self.batch_size - 1) // self.batch_size
        # print('a',a)
        return (self.context_num + self.batch_size - 1) // self.batch_size
        ...
        for batch_i in range((self.context_num + self.batch_size - 1) // self.batch_size):
            ...
            for i, q_seq in enumerate(question_batch):
                for j, id in enumerate(q_seq):
                    ...
                    select_len = min(len(doc), context_len)
                    context_feature[i, j, :select_len, :feature_len] = torch.Tensor(doc[:select_len])
                    #改输入句子：[0,1, 2,3] range(self.start_dialog_ctx,self.end_dialog_ctx)
                    for prv_ctx in range(self.start_dialog_ctx, self.dialog_ctx):
                        if j > prv_ctx:
                            prv_id = id - prv_ctx - 1
...
def leave_one_out_max(prediction, ground_truths):
    ...
    if len(scores_for_ground_truths) == 1:
        # print(scores_for_ground_truths[0])
...
def find_best_score_and_thresh(pred, truth, no_ans_score, min_F1=0.4):
    ...
    for p, t, n in zip(pred, truth, no_ans_score):
        ...
        #8.10
        aa=leave_one_out_max(p, clean_t)
        print(aa)
...
def score(model_results, human_results, min_F1=0.4):
    ...
    # print('model_results',len(model_results))
    # print('human_results',len(human_results))
    ...
    for human_diag_ans, model_diag_ans in zip(human_results, model_results):
        i=i+1
        # print('i',i)
        # print('human_diag_ans',human_diag_ans)
        # print('model_results',model_diag_ans)
        good_dialog = 1.0

        assert len(human_diag_ans) == len(model_diag_ans)
        # print(len(human_diag_ans))
        # print(len(model_diag_ans))
        for human_ans, model_ans in zip(human_diag_ans, model_diag_ans):
            ...
            # print(k)
            #8.24
            # print(machine_f1)#每个的分数
            #8,224
            # F1[j].append(machine_f1)
...
#       for qa in qa_list:
#         q_idx = qa['id']
#         ynfollowup=qa['followup']
#         # print('ynfollowup',ynfollowup)
#         # if ynfollowup=='m':
#         # if ynfollowup=='y'or'm':
#         if ynfollowup!='n':
#         # if ynfollowup=='n'or'm':
#           yynfollowup=ynfollowup
#           i=i+1
#           # print('ynfollowup',ynfollowup)
