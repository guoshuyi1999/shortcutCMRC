...
def main():
    ...
    if args.resume:
        batches = BatchGen_CoQA(dev, batch_size=args.batch_size, evaluation=True, gpu=args.cuda, start_dialog_ctx=args.start_dialog_ctx,dialog_ctx=args.explicit_dialog_ctx)
        predictions = []
    ...
    for epoch in range(epoch_0, epoch_0 + args.epoches):
        ...
        # train 改
        batches = BatchGen_CoQA(train, batch_size=args.batch_size, gpu=args.cuda, start_dialog_ctx=args.start_dialog_ctx,dialog_ctx=args.explicit_dialog_ctx, precompute_elmo=args.elmo_batch_size // args.batch_size, prob_shuffle=args.prob_shuffle)
        start = datetime.now()
        ...
        if epoch % args.eval_per_epoch == 0:
            batches = BatchGen_CoQA(dev, batch_size=args.batch_size, evaluation=True, gpu=args.cuda, start_dialog_ctx=args.start_dialog_ctx , dialog_ctx=args.explicit_dialog_ctx, precompute_elmo=args.elmo_batch_size // args.batch_size)
            predictions = []
            for batch in batches:
