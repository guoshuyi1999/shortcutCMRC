...
#加的
parser.add_argument('--dev_file', type=str, default='CoQA_data/dev.json')
parser.add_argument('--output_dir', type=str, default='CoQA_data/')
...
def proc_train(ith, article):
    ...
    for j, (question, answers) in enumerate(zip(article['questions'], article['answers'])):
        ...
        q_text = question['input_text']   #1.只有自己本身的question
        # print('1q_text',q_text)
        # #如果存在no_prepend_answer 就不进入循环，就不在q_text中加入pre_ans
        if j > 0 and not args.no_prepend_answer:     #2.加上prev-answer 的question
            q_text = article['answers'][j-1]['input_text'] + " // " + q_text
            # print('2q_text',q_text)
        if j>0 and not args.no_prepend_answer and args.no_now_question: #3.存在过去的ans 但不存在本身的question
            q_text = article['answers'][j-1]['input_text'] 
        #     print('3q_text',q_text) 
    ...
    # print('rows(ith, q_text, answer, answer_start, answer_end, rationale, rationale_start, rationale_end, answer_choice):',rows)
    # print('context:',context)
    return rows, context
...
def generate_ablated_input(option, parsed_doc, parsed_query):
    ...
    if option == "drop_logical_words":
        ...
        i1,j1,dropped_doc, mask_dict_c= mask_tokens(parsed_doc, logical_words)
        i2,j2,dropped_query, mask_dict_q= mask_tokens(parsed_query, logical_words)
        ...
        log.info('doc-drop {0}/{1} .'.format(i1,j1))
        log.info('qestion-drop {0}/{1} .'.format(i2,j2))
        ...
        # )
        #context文章
        i1,j1,dropped_doc, mask_dict_c= mask_tokens(parsed_doc, pronouns)

        # i2,j2,dropped_query, mask_dict_q = mask_tokens(parsed_query, pronouns)
        log.info('doc-drop {0}/{1} inconsistent samples.'.format(i1,j1))
        # log.info('qestion-drop {0}/{1} inconsistent samples.'.format(i2,j2))
        ...
        interrogatives = ["what", "when", "where", "who", "which", "why", "whom", "how"]
        i1,j1,dropped_doc, mask_dict_c= mask_tokens(parsed_doc, interrogatives)#加上的
        i2,j2,dropped_query, mask_dict_q= mask_tokens(
...
if input_ablation==1:
    ...
    log.info('12:drop_logical_words')
if input_ablation==2:
...
#     j1=ablated_example["doc-j1"]
#8.18
# i2=ablated_example["q-i2"]
# j2=ablated_example["q-j2"]
...
# print('trC_unnorm_tokens',trC_unnorm_tokens)
