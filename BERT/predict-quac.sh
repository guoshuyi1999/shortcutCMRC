...
do
    for model in {3-pre,bert,mask-log,mask-pro,mask-no-wh,mask-only-wh,no-text,no-conv,no-position,no-qt,only-at-1,only-frist,only-second,only-third}; do
        model_dir="../bert-models-quac/${seed}/${model}"
        cmd=(python test.py
             "${model_dir}/config.json"
             "${model_dir}/model.pkl.2"
             "../data/quac/quac-bert/valid.pkl"
...
do
    for model in {3-pre,bert,mask-log,mask-pro,mask-no-wh,mask-only-wh,no-text,no-conv,no-position,no-qt,only-at-1,only-frist,only-second,only-third}; do
        ...
        cmd=(python test.py
             "${model_dir}/config.json"
             "${model_dir}/model.pkl.2"
             "../data/quac/bert-dev-attack/valid.pkl"
    ...
    # model_dir="../bert-models-quac/${seed}/bert"
    # cmd=(python test.py
