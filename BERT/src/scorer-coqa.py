...
def display_counter(title, c, c2=None):
  ...
  for key, _ in c.most_common():
    if c2:
      print('%s: %d / %d, %.3f%%, F1: %.3f' % (
        key, c[key], sum(c.values()), c[key] * 100. / sum(c.values()), sum(c2[key]) * 100. / len(c2[key])))
    else:
      print('%s: %d / %d, %.3f%%' % (key, c[key], sum(c.values()), c[key] * 100. / sum(c.values())))
...
def eval_fn(val_results, model_results, verbose):
  ...
  print("=======================")
  print('Overall F1: %.3f' % overall_f1)
  print('Yes/No Accuracy : %.3f' % yesno_score)
  print('Followup Accuracy : %.3f' % followup_score)
  ...
  print('Human F1: %.3f' % (100.0 * sum(human_f1) / len(human_f1)))
  print('Model F1 >= Human F1 (Questions): %d / %d, %.3f%%' % (HEQ, total_qs, 100.0 * HEQ / total_qs))
  print('Model F1 >= Human F1 (Dialogs): %d / %d, %.3f%%' % (DHEQ, total_dials, 100.0 * DHEQ / total_dials))
  print("=======================")
  return metric_json

if __name__ == "__main__":
  parser = ArgumentParser()
  parser.add_argument('--val_file', type=str, required=True, help='file containing validation results') #包含验证结果的文件'
  parser.add_argument('--model_output', type=str, required=True, help='Path to model output.')          #模型输出路径
  parser.add_argument('--o', type=str, required=False, help='Path to save score json')                  #保存score json的路径
  parser.add_argument('--min_f1', type=float, default=0.4, help='file containing validation results')   #包含验证结果的文件'
  parser.add_argument('--verbose', action='store_true', help='print individual scores')                 #打印个人成绩的
